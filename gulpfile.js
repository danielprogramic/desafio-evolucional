...
gulp.task('build', ['clean', 'scripts', 'sass', 'templates', 'minify:images', 'browserify', 'copy:vendor:css', 'copy:index', 'copy:assets', 'copy:data']);
...
/**
 * Copies the contents of data folder into the build folder.
 */
gulp.task('copy:data', function() {
  return gulp.src(config.data.sources)                    // Get assets (e.g favicon, logo, shared images etc.)
    .pipe(gulp.dest(config.data.destinationFolder));      // Copy them to the destination folder (e.g assets)
});
